...
export class MemoryRepository implements MemoryRepoInterface {
  ...
  constructor(private db: Database.Database) {
    this.stmts = {
      ...
      findByKey: this.db.prepare(
        'SELECT * FROM memories WHERE project_id IS ? AND key = ? AND active = 1'
      ),
      ...
      findByCategoryProject: this.db.prepare(
        'SELECT * FROM memories WHERE category = ? AND project_id = ? AND active = 1 ORDER BY importance DESC LIMIT ?'
      ),
      ...
      findActiveProject: this.db.prepare(
        'SELECT * FROM memories WHERE active = 1 AND project_id = ? ORDER BY updated_at DESC LIMIT ?'
      ),
  ...
  findByKey(projectId: number | null, key: string): MemoryRecord | undefined {
    return this.stmts.findByKey.get(projectId ?? null, key) as MemoryRecord | undefined;
  }

  findByCategory(category: MemoryCategory, projectId?: number, limit: number = 50): MemoryRecord[] {
    if (projectId !== undefined) {
      return this.stmts.findByCategoryProject.all(category, projectId, limit) as MemoryRecord[];
    }
    return this.stmts.findByCategoryAll.all(category, limit) as MemoryRecord[];
  }

  findActive(projectId?: number, limit: number = 50): MemoryRecord[] {
    if (projectId !== undefined) {
      return this.stmts.findActiveProject.all(projectId, limit) as MemoryRecord[];
    }
