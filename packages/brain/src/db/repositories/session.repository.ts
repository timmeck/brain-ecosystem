...
export class SessionRepository implements SessionRepoInterface {
  ...
  constructor(private db: Database.Database) {
    this.stmts = {
      create: this.db.prepare(`
        INSERT INTO sessions (session_id, project_id, started_at, ended_at, summary, goals, outcome, metadata, embedding)
        VALUES (@session_id, @project_id, @started_at, @ended_at, @summary, @goals, @outcome, @metadata, @embedding)
      `),
  ...
  findByProject(projectId: number, limit: number = 20): SessionRecord[] {
    return this.stmts.findByProject.all(projectId, limit) as SessionRecord[];
  }
