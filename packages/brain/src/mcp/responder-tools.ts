...
function registerResponderToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'brain_responder_status',
    'Get the AutoResponder status — shows how many anomalies Brain has automatically responded to, success rate, and recent actions.',
    {},
    async () => {
  ...
  server.tool(
    'brain_responder_history',
    'Get history of automatic responses Brain has taken in reaction to anomalies.',
    {
  ...
  server.tool(
    'brain_responder_rules',
    'List the active response rules that Brain uses to automatically react to anomalies.',
    {},
    async () => {
