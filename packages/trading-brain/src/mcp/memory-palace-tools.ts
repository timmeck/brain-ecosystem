...
function registerMemoryPalaceToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_palace_status',
    'Get Trading Brain MemoryPalace status: knowledge graph stats, recent connections, top connected nodes.',
    {},
    async () => {
      const status: AnyResult = await call('palace.status', {});
      const lines = [
        '# Trading Memory Palace Status',
        '',
  ...
  server.tool(
    'trading_palace_map',
    'Get Trading Brain knowledge map: connected knowledge nodes and relationships.',
    ...
    async (params) => {
      ...
      const lines = [`# Trading Knowledge Map — ${map.nodes.length} nodes, ${map.edges.length} edges`];
  ...
  server.tool(
    'trading_palace_path',
    'Find shortest path between two knowledge nodes in Trading Brain memory.',
  ...
  server.tool(
    'trading_palace_build',
    'Trigger Trading Brain MemoryPalace to auto-detect knowledge connections.',
    {},
    async () => {
