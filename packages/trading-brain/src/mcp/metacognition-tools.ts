...
function registerMetacognitionToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_metacognition_status',
    'Get Trading Brain meta-cognition status: engine report cards (A-F grades), frequency adjustments.',
    {},
    async () => {
      const status: AnyResult = await call('metacognition.status', {});
      const lines = ['# Trading Meta-Cognition Status', '', `**Engines:** ${status.totalEngines} | **Metrics:** ${status.cycleMetrics}`, ''];
  ...
  server.tool(
    'trading_engine_report',
    'Get detailed report card for a Trading Brain engine.',
  ...
  server.tool(
    'trading_auto_experiment_status',
    'Get Trading Brain auto-experiment status: running experiments, candidates, results.',
    {},
    async () => {
      const status: AnyResult = await call('autoexperiment.status', {});
      const lines = ['# Trading Auto-Experiments', '', `Total: ${status.totalExperiments} | Running: ${status.running} | Adopted: ${status.adopted} | Rolled Back: ${status.rolledBack}`, ''];
  ...
  server.tool(
    'trading_parameter_registry',
    'View Trading Brain tunable parameters with values and bounds.',
    ...
    async (params) => {
      ...
      const lines = ['# Trading Parameter Registry', '', `**Parameters:** ${allParams?.length ?? 0}`, ''];
