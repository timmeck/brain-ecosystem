...
function registerGoalToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_goal_status',
    'Get Trading Brain goal tracking status: active/achieved/failed goals.',
    {},
    async () => {
  ...
  server.tool(
    'trading_goal_create',
    'Create a new goal for Trading Brain to track autonomously.',
    {
  ...
  server.tool(
    ...
    'Get detailed progress for a Trading Brain goal with forecast.',
    { goalId: z.number() },
    async (params) => {
