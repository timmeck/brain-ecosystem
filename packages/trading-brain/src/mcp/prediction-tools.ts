...
function registerPredictionToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_predict',
    'Generate a trading prediction for a specific metric. Uses Holt-Winters / EWMA with auto-calibration. Predicts win rates, PnL trends, signal counts.',
    {
      metric: z.string().describe('Metric name (e.g. "trade_win_rate", "trade_pnl", "signal_count")'),
      domain: z.enum(['error', 'trade', 'engagement', 'metric', 'custom']).optional().describe('Prediction domain (default: trade)'),
  ...
  server.tool(
    'trading_prediction_accuracy',
    'Get trading prediction accuracy statistics: hit rate, mean error, calibration. Shows how good the trading brain is at forecasting.',
    {
    ...
    async (params) => {
      ...
      const lines = ['Trading Prediction Accuracy:\n'];
  ...
  server.tool(
    'trading_predictions_list',
    'List recent trading predictions with status (pending/correct/wrong/partial/expired).',
    {
    ...
    async (params) => {
      ...
      const lines = [`Trading Predictions (${predictions.length}):\n`];
