...
function registerEvolutionToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_evolution_status',
    'Get Trading Brain evolution engine status: generation, population, best fitness, champion.',
    {},
    async () => {
      const status: AnyResult = await call('evolution.status', {});
      const lines = [
        '# Trading Evolution Status',
        '',
  ...
  server.tool(
    'trading_evolution_history',
    'Get Trading Brain evolution generation history.',
    ...
    async (params) => {
      ...
      const lines = ['# Trading Evolution History', '', '| Gen | Best | Avg | Diversity |', '|-----|------|-----|-----------|'];
  ...
  server.tool(
    'trading_evolution_best',
    'Get the best-performing individual from Trading Brain evolution.',
    {},
    async () => {
      const best: AnyResult = await call('evolution.best', {});
      if (!best) return textResult('No individuals yet.');
      const lines = [`# Trading Champion (Gen ${best.generation})`, '', `**Fitness:** ${best.fitness.toFixed(3)} | **Mutations:** ${best.mutationCount}`];
