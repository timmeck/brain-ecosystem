...
function registerAttentionToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_focus_status',
    'Get Marketing Brain attention status: current work context, top topics, urgent topics, engine weights.',
    {},
    async () => {
  ...
  server.tool(
    'marketing_focus_set',
    'Direct Marketing Brain attention to a specific topic (e.g., "content_publishing", "anomaly_detection").',
    {
  ...
  server.tool(
    'marketing_focus_history',
    'Get Marketing Brain focus timeline and context switch history.',
