...
function registerEmergenceToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_emergence_status',
    'Get Marketing Brain emergence status: emergent events, complexity metrics, surprise scores.',
    {},
    async () => {
      const status: AnyResult = await call('emergence.status', {});
      const lines = ['# Marketing Emergence Status', '', `**Events:** ${status.totalEvents} (${status.unpredictedCount} unpredicted)`, `**Avg Surprise:** ${(status.avgSurpriseScore * 100).toFixed(0)}%`];
  ...
  server.tool(
    'marketing_emergence_detect',
    'Scan for emergent patterns in Marketing Brain.',
    {},
    async () => {
  ...
  server.tool(
    'marketing_complexity_metrics',
    'Compute Marketing Brain complexity: Kolmogorov, Shannon entropy, Phi, network density.',
    {},
    async () => {
  ...
  server.tool(
    'marketing_emergence_journal',
    'View Marketing Brain emergence journal.',
